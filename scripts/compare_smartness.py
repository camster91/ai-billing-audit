...
def _encounter_level_diff(
    ...
    common = set(base_by_eid) & set(new_by_eid)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare two smartness-test JSON files")
    ...
    print(f"Micro-aggregate:")
    print(f"              P       R       F1       TP / predicted / gold")
    print(f"  base:      {bm['precision']:.3f}  {bm['recall']:.3f}  {bm['f1']:.3f}  "
          f"{bm['true_positives']} / {bm['total_predicted']} / {bm['total_gold']}")
    print(f"  new:       {nm['precision']:.3f}  {nm['recall']:.3f}  {nm['f1']:.3f}  "
    ...
    print("  ... and {} more (use --detailed to see all)".format(max(0, len(deltas) - 10)))
    print()
    ...
    blat = bs["latency"]
    nlat = ns["latency"]
    print(f"Latency: base mean={blat['mean_s']:.2f}s p95={blat['p95_s']:.2f}s  "
