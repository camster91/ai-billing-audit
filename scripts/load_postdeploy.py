...
TASKS = [
    ...
    (
        10,
        "Verify v12 prompt is in the live container image",
    ...
    (
        10,
        "Document the marketing site (apps/portal) deploy gap",
    ...
    (
        5,
        "Restore the caddy rate_limit directive via a custom caddy build",
    ...
    (
        5,
        "Add per-deploy smoke test to deploy-to-vps.sh",
    ...
    (
        5,
        "Run val_ca.json smoke test on the live container",
    ...
    (
        3,
        "Add cron job to rotate the Traefik dynamic config daily",
    ...
    (
        3,
        "Remove the apps/portal .env.bak file from the live container image",
    ...
    (
        3,
        "Document the deploy chat-layer-redacting issue",
    ...
    (
        3,
        "Add a 5xx error rate alert for the live URL",
    ...
    (
        1,
        "Document the apps/portal deploy story in CLAUDE.md / AGENTS.md / README",
    ...
    (
        1,
        "Clean up the residual post-deploy audit tasks on prior boards",
    ...
    (
        1,
        "Add SOMB-fee primary-source corroboration (the albertadoctors Fee Navigator pull)",
    ...
    (
        1,
        "Wire the deferred monthly_report task to monthly_report.py module",
    ...
    (
        3,
        "Wait for first AHCIP pilot to sign, then unblock defensible revenue numbers",
...
def load():
    ...
    for pri, title, body in TASKS:
        idempotency_key = (
            f"postdeploy-p{pri}-{hash(title) & 0xFFFFFF}-{hash(body[:100]) & 0xFFFFFF}"
        )
        r = subprocess.run(
            [
                "hermes",
