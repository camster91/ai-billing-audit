...
def _finding(
    severity: str, finding_id: str = "F-1", rule_id: str = "rule_modifier_25_001"
):
    return {
        ...
        with (
            ...
            sent = job_queue._send_doctor_emails(
                ...
                findings=[
                    ...
                    _finding(
                        "critical", finding_id="F-crit", rule_id="rule_modifier_25_001"
                    ),
...
def test_send_returns_zero_when_summary_unbuildable():
    ...
    with (
        patch("ai_billing_audit.doctor_email.build_doctor_summary", return_value=None),
        patch(
            "ai_billing_audit.doctor_email.send_doctor_summary", return_value=True
        ) as send,
    ):
