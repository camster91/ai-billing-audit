...
export async function PUT(request: Request, context: RouteContext) {
  ...
  try {
    const task = await updateCompanyTask(id, taskId, companyTaskMutationSchema.parse(body), gate.request);
    return NextResponse.json({ ok: true, task: { id: task.id, version: task.version } });
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: "invalid_input", details: error.flatten() }, { status: 400 });
    }
    if (error instanceof CompanyTaskError) {
      const status = error.code === "task_not_found" ? 404 : error.code.includes("conflict") ? 409 : 400;
      return NextResponse.json({ error: error.code, detail: error.message }, { status });
    }
    return internalErrorResponse(request, error, `/api/hq/clients/${id}/tasks/${taskId}`);
  }
}
