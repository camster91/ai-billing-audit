...
- **Mixed pattern, mostly correct.**
  ...
  - `environment:` blocks (api lines 53-58, worker lines 86-91) explicitly set `LLM_PROVIDER`, `MINIMAX_BASE_URL`, `DATABASE_URL`, `AUDIT_TRAIL_DB`, `LOG_LEVEL` with `${VAR:-default}` patterns. **`MAILGUN_API_KEY` is NOT in the environment block on either service.** This is the silent-fallback-to-dev-mailbox root cause for live doctor emails.
...
5. **Inject `MAILGUN_API_KEY` into the api/worker environment** by reading it from a host secret file the same way `LLM_API_KEY` is handled. Until this is done, all live doctor summary emails are silently being filed in `/app/logs/doctor_emails.jsonl` on the prod container.
...
- `doctor_emails.jsonl` — fallback mailbox when `MAILGUN_API_KEY` unset, written by `src/ai_billing_audit/doctor_email.py:64` (`_LOGS_DIR / "doctor_emails.jsonl"`) and line 559. Confirmed fallback path.
...
| 3 | `MAILGUN_API_KEY` not set on live container | **CONFIRMED by code reading, not by live inspection** | `docker-compose.yml:51-58` (api) and 84-91 (worker) define an `environment:` block that includes `LLM_PROVIDER`, `MINIMAX_BASE_URL`, `DATABASE_URL`, `AUDIT_TRAIL_DB`, `LOG_LEVEL` — **but not `MAILGUN_API_KEY`**. The `env_file: - .env` (lines 51, 84) reads the host's `.env`, but `deploy-to-vps.sh:135-151` writes `LLM_API_KEY`, `MINIMAX_API_KEY`, `OPENAI_API_KEY`, `POSTGRES_PASSWORD` to the host's `.env` and **does not write `MAILGUN_API_KEY`**. There is no host secret file pattern for Mailgun analogous to `/root/ai-billing-audit-secrets/llm_api_key`. Conclusion: `MAILGUN_API_KEY` is not present in the live container's environment, and `src/ai_billing_audit/doctor_email.py:175-178` will return `None` from `_mailgun_configured()` for every request, falling through to the `/app/logs/doctor_emails.jsonl` mailbox. |
...
| 6 | `doctor_email.py` has dev fallback at `/app/logs/doctor_emails.jsonl` when `MAILGUN_API_KEY` unset | **CONFIRMED** | `src/ai_billing_audit/doctor_email.py:59` defines `_LOGS_DIR = Path("/app/logs")`. Line 64 returns `_LOGS_DIR / "doctor_emails.jsonl"`. Line 175-178 returns `None` from `_mailgun_configured()` when `MAILGUN_API_KEY` is missing. Line 559 does `_LOGS_DIR.mkdir(parents=True, exist_ok=True)` and appends a record. The fallback path is hardcoded and active. |
...
| 2 | **Inject `MAILGUN_API_KEY` into the deploy path** (host secret file + `.env` write + `docker-compose.yml` `MAILGUN_API_KEY` entry). Right now all live doctor emails are silently dropped to a JSONL on the prod container. | `deploy-to-vps.sh`, `docker-compose.yml` | 30 min | High — restores a primary product feature (live doctor summary emails) |
...
| `docker-compose.yml` | 162 | All named volumes (good). Worker has no healthcheck. `POSTGRES_PASSWORD:-audit` fail-open. No resource limits. `MAILGUN_API_KEY` absent. |
...
| `src/ai_billing_audit/doctor_email.py:175-178, 64, 559` | 4 hits | Dev fallback to `/app/logs/doctor_emails.jsonl` confirmed active when `MAILGUN_API_KEY` unset. |
