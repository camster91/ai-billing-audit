...
## 8. v2 deferred — the learning loop

**Status:** Deferred. Not part of the v1 Alberta pilot. Activation is
conditional on the prerequisites below.

### 8.1 What v2 adds (the learning loop)

The v1 pilot ships a pre-submit audit that catches errors before they
hit AHCIP. v2 closes the loop: every biller decision (accept, dismiss,
modify), every appeal outcome (filed, won, lost, withdrawn), and every
denial pattern is fed back into the model so it gets **better at this
specific clinic's patterns** over time.

Concretely, v2 enables:

- **Per-clinic F1 dashboard** — biller can see whether the model is
  getting more accurate for *their* claims month over month. If F1 is
  climbing, the model is learning. If it's flat, the prompt isn't
  tuning and we need to revisit the rule set.
- **Rejection-pattern detection** — when a biller consistently dismisses
  a finding type for a clinic (e.g. always dismisses
  `em_level_upcode` for follow-up visits), v2 down-weights that rule
  in future runs. Avoids the "model is overcalling" failure mode.
- **First-pass approval rate (FPAR) KPI** — the live biller-trust
  metric. Of the last 50 audits the biller accepted unchanged, what
  percentage? High = the model is well-calibrated for this clinic.
  Low = recalibration needed. (Design: `docs/APPROVAL_RATE_KPI.md`.)
- **Appeal-letter win/loss feedback** — letters that won feed the
  prompt, letters that lost get analyzed. The appeal_letter module
  improves over time.
- **Cross-tenant denial pattern recognition (anonymized)** — aggregate
  denial patterns across all Zorva-using clinics (no PHI/PII) to
  predict denial likelihood and typical denial reasons. (Design:
  `docs/CROSS_TENANT_DENIALS.md`.) Highest-leverage capability,
  highest-privacy risk — gated on HIA + PIPEDA + AHS review.
- **Per-specialty prompt fine-tuning** — family medicine billers and
  cardiology billers see different patterns. Once a specialty has 3+
  months of feedback, run MIPROv2 to produce a specialty-tuned prompt
  variant.

### 8.2 Prerequisites (all must be true before v2 activates)

1. **3+ months of pilot data** — a single anchor clinic must have
   submitted at least 3 months of audited encounters through the v1
   pipeline. Without volume, the F1 dashboard is noise.
2. **Per-clinic feedback log** — biller accept/dismiss/modify actions
   must be persisted with finding ID, action type, user, timestamp,
   and (for modify) prior/new values. The feedback log is the seed
   data for the learning loop. (Implementation:
   `src/ai_billing_audit/feedback.py`.)
3. **Appeal outcomes recorded** — every appeal letter generated by
   Zorva must have an outcome (won / lost / withdrawn) logged. Without
   appeal outcomes, the appeal_letter module cannot learn what
   letter styles win.

The cross-tenant capability additionally requires the §7 privacy
review checklist in `docs/CROSS_TENANT_DENIALS.md` to be fully
signed off (HIA Privacy Impact Assessment + PIPEDA de-identification
review + AHS Information Privacy & Security Framework review).

### 8.3 Expected timeline (conditional)

| Milestone | Target | Gate |
|---|---|---|
| First anchor pilot signed | July 2026 | — |
| v1 audit + biller actions live | August 2026 | — |
| 3 months of pilot data accumulated | October 2026 | Prereq 1 |
| Per-clinic feedback log deployed | August 2026 (parallel to v1) | Prereq 2 |
| Appeal outcomes logged | September 2026 (depends on AHCIP remittance cycle) | Prereq 3 |
| v2 learning loop features activated | Q4 2026 (October–December) | All three prereqs true |
| Cross-tenant capability in production | Q1 2027 (earliest) | HIA + PIPEDA + AHS review signed off |

**v2 does not activate on the calendar — it activates on the
prerequisites.** If the pilot slips to August, the timeline shifts
proportionally. If we cannot get the privacy review for the
cross-tenant capability, that feature stays deferred while the
single-clinic learning loop ships.

### 8.4 Why v2 is deferred from the v1 pitch

The v1 pitch to Alberta clinics is the **denial-catch** value prop:
"catches 6-7 of 10 real billing errors before submission" (F1=0.690,
defensible, no leakage inflation). v2 is the **trust-building**
value prop: "gets smarter about YOUR patterns over 90 days." Both
are real, but v1 is what gets a clinic to sign the pilot. v2 is what
keeps them. We do not pitch v2 to the first anchor — we earn the
right to it by shipping v1 well.
