...
- **PHI flows into Mailgun subject/body** through `doctor_email.build_doctor_summary()` — the `patient_label` is interpolated into the email body verbatim (`src/ai_billing_audit/doctor_email.py:412`). The default is the literal string `"the patient"`, but the caller can substitute a real name; the function performs no validation.
...
| S-9 | `MAILGUN_API_KEY` | Mailgun API key (live container status: **session memory says "not yet set on live container"**) | Not present in `pyproject.toml`, `docker-compose.yml`, `.env.example`, or any tracked file. Must be supplied at deploy time. | not in repo | Confirm the next deploy injects it from a host secret file (parallel to the `LLM_API_KEY_FILE` pattern in S-6). Currently `src/ai_billing_audit/doctor_email.py:175` falls back to `doctor_emails.jsonl` when `MAILGUN_API_KEY` is unset — the **fail-open path is the dev path**: emails accumulate on disk unencrypted, which is PHI at rest. |
...
| S-14 | `src/ai_billing_audit/doctor_email.py:175` | `MAILGUN_API_KEY` | Sourced via `os.environ.get`. Not present in any tracked file. | env-var name only | See S-9. |
...
| `/app/logs/doctor_emails.jsonl` | Failed-delivery fallback. Full email body (which contains `patient_label`). | PHI when Mailgun key is missing. | `doctor_email.py:559–569` |
| `/app/logs/npi_email_cache.json` | Caches NPI registry lookups (provider emails keyed by NPI). | Provider PII (not patient PHI). | `doctor_email.py:588–628` |
| `/app/logs/appeal_letters.jsonl` | Metadata only (`encounter_id`, `tenant_id`, `market`, `compliance_law`, `appeal_basis`, `cited_rule_ids`, `requested_action`, `template_only`, `generated_at`). **No letter body.** | Pseudonymized. | `appeal_letter.py:506–518` |
| Mailgun (live send) | Full doctor-summary email with `patient_label` interpolated into the body. | Direct PHI when `patient_label` is a real name. | `doctor_email.py:410–421` |
...
I checked `/tmp/` and the repo for `/tmp/`-style scratch paths. The repo's only log paths are under `/app/logs/` (and `logs/` on the dev host). No `/tmp/` scripts in `scripts/` or `tests/` write patient-derived content to a world-readable path. The Mailgun **fallback path** (`/app/logs/doctor_emails.jsonl`) is the closest equivalent and is PHI-bearing.
...
| K-2 | Mailgun key not set on live container. | **Confirmed by absence.** `MAILGUN_API_KEY` does not appear in any tracked file (`grep -r MAILGUN_API_KEY` returns only `src/ai_billing_audit/doctor_email.py` reads; no writes in compose or pyproject). The doctor_email fallback path (`/app/logs/doctor_emails.jsonl`) is the active path. | `docker-compose.yml`, `pyproject.toml`, `.env.example` (only `apps/portal/.env.example` exists for the portal, not the FastAPI stack). |
...
| A04 — Insecure Design | ✗ | `tenant_delete()` is a documented no-op (`api.py:1989–1995`), defeating right-of-erasure. `AUDIT_ALLOW_NO_AUTH=1` is shipped as a "demo bypass" but is not gated behind a build-time flag. Mailgun fallback (`/app/logs/doctor_emails.jsonl`) accumulates PHI when the API key is unset — fail-open by design. |
...
| `/Users/biancabienaime/projects/ai-billing-audit/src/ai_billing_audit/doctor_email.py` | Mailgun sender + doctor summary |
