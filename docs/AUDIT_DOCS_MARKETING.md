...
| **"Alerts the physician to critical findings"** | Not in portal copy. Mentioned in `src/ai_billing_audit/doctor_email.py`. | `src/ai_billing_audit/doctor_email.py` exists. **Refactored 2026-06-26: auto-send removed.** The summary is now appended to `/app/logs/doctor_emails.jsonl` for the biller to dispatch via their own mail client. Wired into `_default_runner` at `src/ai_billing_audit/job_queue.py`. Not visible in apps/portal — this is an src-only feature. | Honest: "Builds a 1-sentence plain-English summary for each flag-worthy finding and queues it for the clinic to forward via their own mail client." |
