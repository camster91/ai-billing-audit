...
- **Split:** `val (legacy)` — `data/val.json` (50 encounters, ? gold findings)
- **Prompt:** `v0 (legacy)` — `prompts/v0/auditor_prompt.txt`
- **Production-tracking:** NO — this is a legacy baseline run; the daily cron should produce val_ca/v12 runs by default
